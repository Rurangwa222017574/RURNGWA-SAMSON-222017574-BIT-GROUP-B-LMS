...
/**
 * Servlet implementation class DashboardCards
 */
@WebServlet("/DashboardCards")
public class DashboardCards extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			String data = 	 "<div class='card m-2'>"
							+ "<div class='card-body'>"
							+"<h3 class='text-center'>"+action+" loans</h3>"
			...
			if(action.equals("paid")) {
					...
					+ "  WHERE la.status<>0 AND la.pay_status=1 GROUP BY la.id, la.description, la.date, la.amount, la.status, la.pay_status, "
				...
				rs1 = statement.executeQuery("SELECT la.id AS application_id, "
					...
					+ "SUM(lp.payment_amount) AS payment_amount "
					+ "FROM loan_applications la JOIN students s ON la.st_id=s.id LEFT JOIN loan_payments lp ON la.id=lp.loan_id"
					+ "  WHERE la.status=1 AND la.pay_status=0 GROUP BY la.id, la.description, la.date, la.amount, la.status, la.pay_status, "
					+ "s.id, s.name, s.reg, s.email, s.class");
			}
			...
			while(rs1.next()){
				...
				data += "<tr>"
						...
						+"<td>"+statusText+"</td>"
						+"<td>"+statusPay+"</td>";
				if(rs1.getString("status").equals("1") && rs1.getString("pay_status").equals("0")) {
					data += "<td><div class=\"dropdown d-inline\">\r\n" + 
					...
					"    <div class=\"mb-3\">\r\n" + 
				"      <label for=\"coment\" class=\"form-label\"><b>Amount</b></label>\r\n" + 
					" <input class=\"form-control\" type=\"number\" name=\"amount\" id=\"loan_amount\">\r\n"+
					"    </div>\r\n" + 

					"<input type='hidden' name='id' value="+rs1.getString("application_id")+">"+
					"    <button type=\"submit\" class=\"btn btn-sm btn-primary\">Pay</button>\r\n" + 
					"  </form>\r\n" + 
					"</div>\r\n" + 
					"</td>";
				}
				else if(rs1.getString("status").equals("2") || rs1.getString("pay_status").equals("1")) {
					data += "<td><div class=\"dropdown d-inline\">\r\n" + 
					"  <button class=\"btn btn-secondary btn-sm dropdown-toggle\" type=\"button\" id=\"dropdownMenuButton1\" data-bs-toggle=\"dropdown\" aria-expanded=\"false\" disabled='true'>\r\n" + 
					"    Pay\r\n" + 
					"  </button>\r\n" + 
					"<div class=\"dropdown-menu w-20\">\r\n" + 
					"  <form class=\"form\" action='Pay'>\r\n" + 
					"    <div class=\"mb-3\">\r\n" + 
					"      <label for=\"coment\" class=\"form-label\"><b>Amount</b></label>\r\n" + 
					" <input class=\"form-control\" type=\"number\" name=\"amount\" id=\"loan_amount\">\r\n"+
					"    </div>\r\n" + 
			...
			request.setAttribute("location", "loans");
