...
/**
 * Servlet implementation class MyDashboardCards
 */
@WebServlet("/MyDashboardCards")
public class MyDashboardCards extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if(action.equals("paid")) {
					...
					+ "  WHERE la.status<>0 AND la.pay_status=1 AND la.st_id="+userid+" GROUP BY la.id, la.description, la.date, la.amount, la.status, la.pay_status, "
				...
				rs1 = statement.executeQuery("SELECT la.id AS application_id, "
					...
					+ "SUM(lp.payment_amount) AS payment_amount "
					+ "FROM loan_applications la JOIN students s ON la.st_id=s.id LEFT JOIN loan_payments lp ON la.id=lp.loan_id"
					+ "  WHERE la.status=1 AND la.pay_status=0 AND la.st_id="+userid+" GROUP BY la.id, la.description, la.date, la.amount, la.status, la.pay_status, "
					+ "s.id, s.name, s.reg, s.email, s.class");
			}
			else {
				rs1 = statement.executeQuery("SELECT la.id AS application_id, "
					...
					+ "SUM(lp.payment_amount) AS payment_amount "
					+ "FROM loan_applications la JOIN students s ON la.st_id=s.id LEFT JOIN loan_payments lp ON la.id=lp.loan_id"
					+ "  WHERE la.st_id="+userid+" GROUP BY la.id, la.description, la.date, la.amount, la.status, la.pay_status, "
					+ "s.id, s.name, s.reg, s.email, s.class");
			}
			...
			while(rs1.next()){
				...
				else if(rs1.getString("status").equals("2") || rs1.getString("status").equals("0")) {
					 statusText = "Rejected";
					 statusPay = "<i class='badge badge-info'>N/A</i>";
					 paid="-";
					 unpaid="-";
				...
				if(rs1.getString("pay_status").equals("0") && (statusText != "Rejected")&&(statusText != "Pending")) {
